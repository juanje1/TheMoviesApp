...
fun EmailField(
    ...
    OutlinedTextField(
        modifier = Modifier
            ...
            .testTag(stringResource(R.string.register_email_test)),
        value = registerState.user.email,
        onValueChange = { onFieldChanged(RegistrationField.EMAIL, it) },
        ...
        trailingIcon = {
            if (registerState.errorMessages.containsKey(RegistrationField.EMAIL))
                Icon(
        ...
        isError = registerState.errorMessages.containsKey(RegistrationField.EMAIL)
    )
    if (registerState.errorMessages.containsKey(RegistrationField.EMAIL)) {
        Text(
            text = registerState.errorMessages[RegistrationField.EMAIL]?.let { stringResource(it) } ?: "",
