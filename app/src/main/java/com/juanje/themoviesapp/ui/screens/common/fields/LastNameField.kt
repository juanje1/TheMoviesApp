...
fun LastNameField(
    ...
    OutlinedTextField(
        modifier = Modifier
            ...
            .testTag(stringResource(R.string.register_last_name_test)),
        value = registerState.user.lastName,
        onValueChange = { onFieldChanged(RegistrationField.LAST_NAME, it) },
        ...
        trailingIcon = {
            if (registerState.errorMessages.containsKey(RegistrationField.LAST_NAME))
                Icon(
        ...
        isError = registerState.errorMessages.containsKey(RegistrationField.LAST_NAME)
    )
    if (registerState.errorMessages.containsKey(RegistrationField.LAST_NAME)) {
        Text(
            text = registerState.errorMessages[RegistrationField.LAST_NAME]?.let { stringResource(it) } ?: "",
